...
def wavelet(Y, dt, pad, dj=-1, s0=-1, J1=-1, mother=-1, param=-1):
    ...
    if pad == 1:
        ...
        pot   = int(2 ** (base2 + 1) - n1)
        x = np.concatenate((x,np.zeros(pot)), axis=0)
    ...
    nn = int(n) 
    jshape = int(J1+1) 
    wave = np.zeros(shape=(jshape, nn), dtype=complex) 
    ...
    coi = coi * dt *\
    np.concatenate((np.insert(np.arange((n1 + 1) / 2 - 1), [0], [1E-5]), \
                    np.insert(np.flipud(np.arange(0, n1 / 2 - 1)),\
                              [-1], [1E-5])))  # COI [Sec.3g]
